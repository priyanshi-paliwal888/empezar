...
class OAuthProviderClient(models.Model):
    ...
    token_type = fields.Selection(selection_add=[('jwt', 'JSON Web Token')], ondelete={'jwt': 'cascade'})
    ...
    def _generate_jwt_payload(self, request):
        ...
        data = {
            'exp': str((datetime.utcnow() + timedelta(hours=1)).timestamp()),
            'nbf': str(datetime.utcnow().timestamp()),
            'iss': 'Odoo',
        ...
        if request.client.jwt_scope_id:
            ...
            # user's rights
            scope = request.client.jwt_scope_id.sudo()
    ...
    def get_oauth2_server(self, validator=None, **kwargs):
        ...
        def jwt_generator(request):
            ...
            return jwt.encode(payload, str(request.client.jwt_private_key),algorithm=request.client.jwt_algorithm)
